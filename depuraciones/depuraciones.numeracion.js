...
- Identificar numeraciones pegadas al texto por errores de lectura.
========================================================= */
(function(window){
  "use strict";
  ...
  function codigoEmbebido(v){var m=texto(v).match(/[A-Za-zÁÉÍÓÚÜÑáéíóúüñ]([1-4](?:\.\d+){1,4})(?=\s|[A-Za-zÁÉÍÓÚÜÑáéíóúüñ]|$)/);return m?m[1]:"";}
  ...
  function analizarUnidad(unidad){
    var mapa={},hallazgos=[],ultimoPorPadre={};
    (unidad.contenidos||[]).forEach(function(item,indice){
      var codigo=codigoDe(item.texto);
      if(!codigo){
        var embebido=codigoEmbebido(item.texto);
        if(embebido)hallazgos.push(hallazgo("numeracion_pegada","error","Numeración pegada al texto","Se detectó la numeración "+embebido+" unida a una palabra. Debe separarse y colocarse al inicio del componente.",item,{codigo:embebido,indice:indice}));
        else hallazgos.push(hallazgo("contenido_sin_numeracion","advertencia","Contenido sin numeración","Se recomienda asignar una numeración jerárquica coherente con la unidad.",item,{indice:indice}));
        return;
      }
      var p=partes(codigo);
      ...
      var clave=pa||String(p[0]),actual=p[p.length-1],anterior=ultimoPorPadre[clave];
      if(typeof anterior!=="number"&&actual>1)hallazgos.push(hallazgo("inicio_secuencia_incompleto","advertencia","La secuencia no inicia en uno","El primer componente detectado en esta jerarquía es "+codigo+". Verifica si faltan elementos anteriores.",item,{codigo:codigo}));
      else if(typeof anterior==="number"){
  ...
  NS.Numeracion={analizar:analizar,analizarUnidad:analizarUnidad,codigoDe:codigoDe,codigoEmbebido:codigoEmbebido};
})(window);
